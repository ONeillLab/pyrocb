...
for j in range(ny):
    for i in range(nx):
        ...
        # Essentially, I'm approximating the coordinates of the box by using the difference between the XLAT, and XLON which represent the middle coordinates of the boxes.
        # Top edge
...
alpha_gdf = gpd.GeoSeries([alpha_shape], crs=gdf.crs)

fig, ax = plt.subplots(figsize=(8, 8))
alpha_gdf.plot(ax=ax, color='lightblue', edgecolor='blue', alpha=0.5, zorder=1)
gdf.plot(ax=ax, color='red', markersize=20, zorder=2)
...
def rings_from_alpha_shape(alpha_shape_geom):
    """
    Convert a Shapely Polygon/MultiPolygon (lon, lat order, from Alpha_Shaper)
    into a list of (N, 2) [lon, lat] rings -- one per lobe -- for mask_perim.
    """
    poly = alpha_shape_geom
...
rings = rings_from_alpha_shape(alpha_shape)
...
max_ig = np.nanmax(valid[mask])
lfn = xr.where(tign_new<10**9, ((tign_new - max_ig)/max_ig) , 1)
lfn = xr.where(np.abs(lfn)<0.2, 0 , lfn)


fuel = read_wrffile('NFUEL_CAT', wrfin)
fuel = xr.where(lfn<0, 14, fuel)



# Save configuration changes back into your wrfinput file
write_wrffile('LFN_HIST', lfn, wrfin)
write_wrffile('NFUEL_CAT', fuel, wrfin)
